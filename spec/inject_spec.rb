...
describe 'inject' do
  ...
  context 'testing new inject' do
    ...
    it 'with memo' do
      expect(a.finject(0) { |memo,item| memo + item}).to eq 10
    end

    it 'with different memo' do
      expect(a.finject(1) { |memo,item| memo + item}).to eq 11
    end

    it 'with string array' do
      b = %w{a b c d} 
      expect(b.finject('') { |memo,item| memo + item}).to eq 'abcd'
    end

    it 'with multiplication' do
      expect(a.finject(1) { |memo,item| memo * item}).to eq 24
    end
